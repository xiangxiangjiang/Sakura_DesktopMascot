...
def get_html(url):
    try:
        ...
        r = requests.get(url, timeout=500, headers=headers)
        r.raise_for_status()
        # print(r.status_code)
...
if __name__ == "__main__":
    exeName, ver = 'Sakura_DesktopMascot.exe', ''
    if not argv:
        exeName, ver = argv
    print('\t-==八重樱桌宠==-\n\n')
    if ver != '':
        print('当前版本：' + ver + '\t(若为手动升级请先关闭桌宠应用！！！)\n')
    else:
    ...
    while True:
        ...
        needUpdate = True
        if ver != '':
            needUpdate = float(ver) < float(githubData['ver'])
        if githubData != None and needUpdate:
            Output(2)
            # 下载压缩包
            tempPath = 'temp'
            ...
            Output(0)
            break
        elif githubData != None and not needUpdate:
            print('无需更新！\n')
            Output(0)
        else:
            Output(1)
